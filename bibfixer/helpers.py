...
def update_tex_citations(tex_files: Iterable[Path],
    ...
    if logger:
        if updated_files:
            logger.step_ok("update_tex_citations", f"Updated {updated_files} TeX file(s)")
        else:
            logger.step_ok("update_tex_citations", "No TeX citation rewrites required")
    return updated_files


def sanitize_citation_keys(bib_file: Path, logger: RunLogger | None = None) -> Dict[str, str]:
    ...
    if modified:
        ...
        if logger:
            logger.step_ok("sanitize_keys", f"{bib_file.name}: sanitized {fixed_count} key(s)")
    elif logger:
        logger.step_ok("sanitize_keys", f"{bib_file.name}: no key sanitization needed")
    return key_mapping
...
def standardize_citation_keys(bib_file: Path, logger: RunLogger | None = None) -> Dict[str, str]:
    ...
    if key_mapping:
        write_bib_file(bib_file, bib_database)
        if logger:
            logger.step_ok("standardize_keys", f"{bib_file.name}: standardized {len(key_mapping)} key(s)")
    elif logger:
        logger.step_ok("standardize_keys", f"{bib_file.name}: key standardization not needed")
    return key_mapping
