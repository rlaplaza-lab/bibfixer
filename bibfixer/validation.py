...
def validate_citations(logger: RunLogger | None = None) -> List[str]:
    ...
    if all_issues:
        for issue in all_issues:
            print(issue)
        if logger:
            logger.step_warn("citations", f"Found {len(all_issues)} citation issue(s)")
    ...
    if commented_keys:
        ...
        if len(commented_keys) > 10:
            print("  ...")
    if logger and not all_issues:
        logger.step_ok("citations", f"{total_valid}/{total_citations} citations valid")
    return all_issues
...
def validate_bibliography(logger: RunLogger | None = None) -> bool:
    """Run the complete validation suite and return whether everything passed."""
    citation_issues = validate_citations(logger=logger)
    duplicate_keys = check_duplicate_keys()
    ...
    if logger:
        if duplicate_keys:
            logger.step_warn("duplicate_keys", "Duplicate citation keys detected")
        else:
            logger.step_ok("duplicate_keys", "No duplicate citation keys")
        if doi_count:
            logger.step_warn("duplicate_dois", f"{doi_count} DOI duplicate group(s)")
        else:
            logger.step_ok("duplicate_dois", "No DOI duplicates")
        if title_count:
            logger.step_warn("duplicate_titles", f"{title_count} title duplicate group(s)")
        else:
            logger.step_ok("duplicate_titles", "No duplicate titles")
        if not syntax_ok:
            logger.step_error("syntax", "BibTeX syntax issues detected")
        else:
            logger.step_ok("syntax", "BibTeX syntax checks passed")
        if author_count:
            logger.step_warn("author_fields", f"{author_count} malformed author field(s)")
        else:
            logger.step_ok("author_fields", "No malformed author fields detected")
        if percent_count:
            logger.step_warn("unescaped_percent", f"{percent_count} unescaped percent issue(s)")
        else:
            logger.step_ok("unescaped_percent", "No unescaped percent issues")
