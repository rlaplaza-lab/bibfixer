...
def abbreviate_journal_names(bib_file: Path) -> int:
    ...
    measure of safety when upstream metadata refresh paths are unavailable or
    crash (segfaults, timeouts, etc.).
    ...
    for entry in bib_database.entries:
        ...
        if lookup_key in ci_lookup:
            ...
            # on the mapping; this also covers the case where metadata refresh
...
def uncomment_bibtex_entries(bib_file: Path) -> int:
    """Uncomment BibTeX entries that were commented out by upstream tooling.

    Some formatters may comment out entries with syntax errors. This routine
...
def fix_malformed_author_fields(bib_file: Path) -> int:
    ...
    for entry in bib_database.entries:
        ...
        # Normalize accidental newline escape artifacts produced by some tools.
        # These break LaTeX author parsing and should be plain separators.
        value = value.replace("\\n", " and ")
