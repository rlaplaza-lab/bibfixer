...
def test_uncomment_bibtex_entries(tmp_path):
    ...
    # simple commented entry introduced by formatting pass
...
def test_fix_malformed_author_newline_artifacts(tmp_path):
    bib = tmp_path / "newline-authors.bib"
    bib.write_text("""@article{A,
  author={Doe, Jane\\nSmith, John /n Brown, Bob},
}
""")
    fixed = cli.fix_malformed_author_fields(bib)
    assert fixed == 1
    ...
    assert "\\n" not in out
    assert "/n" not in out
    assert "Doe, Jane and Smith, John and Brown, Bob" in out
